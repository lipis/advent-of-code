...
def get_coordinates(start, segment):
    direction = segment[:1]
    dist = int(segment[1:])
    if direction == "R":
...
for seg in l1:
    line1.append(get_coordinates(line1[-1], seg))
for seg in l2:
    line2.append(get_coordinates(line2[-1], seg))
...
def within(point, line):
    if line[0][1] == line[1][1]:
        ...
        if line[0][1] != point[1]:
            return False
        return mn <= point[0] and point[0] <= mx
    ...
    if line[0][0] == line[1][0]:
        ...
        if line[0][0] != point[0]:
            return False
        return mn <= point[1] and point[1] <= mx
...
def line_intersection(line1, line2):
    ...
    y = det(d, ydiff) / div
    p = x, y
    if not within(p, line1):
        return False
    if not within(p, line2):
        return False
    return x, y
...
for point1 in range(len(line1) - 1):
    for point2 in range(len(line2) - 1):
        first = (line1[point1], line1[point1 + 1])
        second = (line2[point2], line2[point2 + 1])
        intersect = line_intersection(first, second)
        if intersect:
...
def distance(line, point):
    ...
    for p in range(len(line) - 1):
        segment = line[p], line[p + 1]
        if within(point, segment):
            total += dist((segment[0], point))
            return total
        else:
            total += dist(segment)
